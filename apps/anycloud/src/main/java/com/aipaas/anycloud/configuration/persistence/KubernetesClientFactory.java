...
public final class KubernetesClientFactory {
    ...
    /** {@link ClusterEntity} 로부터 새 {@link KubernetesClient} 생성. 호출부는 캐시 컴포넌트 경유 권장. */
    ...
    // kubeconfig YAML 생성은 cluster-agent 의 GENERATE_KUBECONFIG RPC (KubeconfigExportService)
    // 가 담당 — backend 는 admin 자격 미보관.

    /**
     * Stub — backend 는 cluster K8s API 직접 호출 X, 모든 호출은 cluster-agent RPC 로 대체.
     * 호출 시 명확한 에러. 본 클래스 호출처 정리 후 파일 자체 삭제 예정.
     */
    private static Config buildConfig(ClusterEntity cluster) {
        throw new UnsupportedOperationException("KubernetesClientFactory deprecated: backend 는 더 이상 cluster K8s API 를 "
                + "직접 호출하지 않음. cluster-agent RPC (LIST_RESOURCES, APPLY_MANIFEST 등) 사용 권장. "
                + "cluster=" + (cluster == null ? "null" : cluster.getId()));
    }

    /** Base64 인코딩 키의 알고리즘 종류 (EC/RSA/UNKNOWN) 판별. */
    ...
    /** SEC1 형식 EC private key → PKCS#8 PEM 변환. */
