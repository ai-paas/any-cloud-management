 ...
 *
 * <ol>
 *   <li>Day-2 ops routing — {@code kubeResourceService} (agent stream). 모든 list/get/delete 가
 *       이 path 만 사용.</li>
 *   <li>Bootstrap-only fabric8 fallback — {@code bootstrapKubeClient} + {@code clusterRepository}.
 *       cluster 등록 직후 agent 미존재 chicken-and-egg 해소 전용.</li>
 *   <li>Error classification + degraded metric — {@code degradedReasonCounter} + private classifier
 *       methods. 향후 {@code KubeErrorClassifier} 로 추출 가능.</li>
 *   <li>Response sanitization — private {@code sanitize*} methods. {@code service/kube/support/}
...
public class KubeServiceImpl implements KubeService {
    ...
    /** Day-2 K8s ops 를 agent stream 으로 routing — starter 측 facade. 부터 agent-only path. */
    private final KubeResourceService kubeResourceService;
    /** Bootstrap-only fabric8 client cache — cluster 등록 직후 agent 미존재 시 chicken-and-egg 해소. */
     ...
     * mitigation. default ON — production 환경에서 secret base64 value 가 backend response 에 노출되지
     ...
     *
     * <p>fabric8 fall-through 제거 — 모든 day-2 path 가 agent-only.
     ...
     *
     * <p>legacy 경로 (fabric8 + {@link ResourceType} dispatcher) 제거. agent 의
     * {@code LIST_RESOURCES} 를 큰 limit 으로 호출해 동일 결과 산출. caller (legacy /v1 endpoint)
     * 의 페이지 미사용 시그니처를 유지하면서 day-2 path 통합.
     *
     * <p>CircuitBreaker fallback 은 빈 배열 — UI 일부 위젯이 5xx 로 무너지지 않도록 부분 가용성.
     *
    ...
    public JsonNode getResources(String clusterName, String namespace, String kind) {
        ...
        try {
            ...
            // 더 큰 페이지가 필요하면 caller 가 listResourcesPaginated 로 직접 페이징해야 한다.
    ...
    /**
     * Manifest apply (server-side apply) — agent-only. multi-doc YAML / JSON 둘 다 agent 가
    ...
    /**
     * Bootstrap path — ClusterEntity 의 kubeconfig 로 fabric8 client 만들어 server-side apply.
     *
     * <p>호출 시점: cluster 등록 직후. day-2 ops 는 절대 이걸 부르지 않는다. {@link AgentApiManagedInstaller}
     * 가 {@link #applyResource} 가 AGENT_UNAVAILABLE 로 실패할 때만 fallback 으로 사용.
     *
     * <p>fabric8 의 multi-doc YAML 지원으로 manifest 안의 모든 resource 를 한 번에 apply. 마지막
     ...
     *
     * <p>동일 paginated agent path 로 통합. legacy endpoint 의 separate "pods" 시그니처
    ...
    public PagedKubeResourceResponse listResourcesPaginated(
        ...
        try {
            KubeResourcePage page = kubeResourceService.listResourcesPaginated(
                    clusterName, ns, normalizedKind, safeLimit, continueToken, labelSelector);
            JsonNode items = sanitize(page.items());
            log.info(
                    "Resource page loaded (source={}): cluster={}, ns={}, kind={}, limit={}, returned={}, hasNext={}",
                    SRC_AGENT,
                    clusterName,
                    ns,
                    normalizedKind,
    ...
    /**
     * 호환성 alias — {@link com.aipaas.anycloud.domain.kube.model.K8sKinds#CLUSTER_SCOPED} 가 단일
     ...
     * (cluster, kind, reason) 라벨로 누적. FORBIDDEN 다발 cluster 자동 식별 가능. MeterRegistry 미주입
     * 또는 등록 실패는 fail-open (audit 과 동일 정책).
     *
     * <p>kind 카디널리티: GVR 식별 가능한 string 으로 normalize ({@code pods}, {@code applications.argoproj.io} 등).
