...
전부 503 을 반환하므로, 붙이지 않으면 API 표면의 상당 부분을 로컬에서 검증할 수 없다.

실측: Bruno 컬렉션 88개 요청 기준 agent 없이 성공 31건, 붙인 뒤 45건.

## 왜 k3d 로 되는가

agent 는 backend 로 **outbound gRPC** 를 연다. backend 가 cluster 안으로 들어오는 구조가
아니라서 k3d 처럼 로컬 컨테이너 기반 K8s 면 충분하다. VM 이나 별도 네트워크 설정이 필요 없다.
...
기존 클러스터를 쓰면 kubeconfig 나 기존 워크로드와 얽히므로 따로 만든다.
...
로컬에서 빌드해 k3d 로 넣는다. `ghcr.io/aipaas/cluster-agent` 는 공개 이미지가 아니다.
...
응답 `data.bootstrap.token` 이 등록 토큰이다. **유효기간 10분**이라 발급 후 바로 설치한다.
...
**`backend.grpcAddr` 를 반드시 덮어쓴다.** 등록 응답의 `helmInstallCommand` 는 backend 의
`agent.grpc.public-endpoint` 값(기본 `127.0.0.1:9090`)을 넣어주는데, 그 주소는 파드 안에서
자기 자신을 가리켜 연결되지 않는다. `host.k3d.internal` 도 파드 DNS 에서 해석되지 않아
Docker 네트워크 게이트웨이 IP 를 쓴다.
...
k3d 노드를 backend 와 같은 Docker 네트워크에 두면 게이트웨이 IP 대신 서비스명을 쓸 수 있다.
...
IP 가 환경마다 달라지는 문제를 피할 수 있으나, 네트워크 수명주기가 compose 에 묶인다.
