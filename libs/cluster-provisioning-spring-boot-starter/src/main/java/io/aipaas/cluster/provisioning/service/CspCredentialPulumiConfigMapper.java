 ...
 * static credential, {@code gcp:credentials} → GCP service account 등). Pulumi program 자체는
 * 변경 불필요.
 *
 * <p>변환된 stack config 는 {@link io.aipaas.cluster.provisioning.service.PulumiCommandService#setConfig}
 * 로 {@code pulumi config set --secret} 실행 — Pulumi state 의 passphrase 로 암호화 저장.
 */
public final class CspCredentialPulumiConfigMapper {
	 ...
	 * skip (안전 default). 모든 value 는 secret 으로 취급되어야 한다.
	 *
	...
	public static Map<String, String> toPulumiConfig(String provider, Map<String, String> env) {
		...
		if (env == null || env.isEmpty() || provider == null) {
			return out;
		}
		...
		switch (p) {
			...
			case "gcp" -> {
				// GOOGLE_CREDENTIALS inline JSON 우선, 없으면 APPLICATION_CREDENTIALS path
				// (path 는 backend container 안에 mount 된 경우만 의미. 대부분 inline JSON).
				String inline = env.get("GOOGLE_CREDENTIALS");
				if (inline != null && !inline.isBlank()) {
					put(out, "gcp:credentials", inline);
				} else {
					put(out, "gcp:credentials", env.get("GOOGLE_APPLICATION_CREDENTIALS"));
				}
			}
			case "azure" -> {
			...
			case "digitalocean" -> {
				...
				if (tok == null || tok.isBlank()) {
					tok = env.get("DIGITALOCEAN_ACCESS_TOKEN");
				}
				put(out, "digitalocean:token", tok);
			}
			default -> {
				// unknown provider — skip (caller 가 validation 단계에서 이미 reject).
			}
		}
		return out;
	}
	 ...
	 * (compose env_file 같은 process-level) 에서 별도 set 되어야 한다.
	 */
	...
	public static Map<String, String> stripCspEnv(Map<String, String> env) {
		...
		for (Map.Entry<String, String> e : env.entrySet()) {
			if (!CSP_ENV_BLOCKLIST.contains(e.getKey())) {
