 ...
 * 정합성을 강제한다. 새 provider 가 누락된 필드를 export 하면 ProvisioningOutputMapper (host-side)
 * 가 부팅/실행 시점에 {@link ProvisioningOutputValidationException} 을 던져 즉시 인지된다.
 *
 * <p>8 CSP 모두에 공통인 키만 record 필드로 두고, provider 별 추가 필드는 {@code providerNative} Map 에
 * 격리하여 호환성을 보존한다.
 *
 * @param provider          정규화된 provider 이름 (예: aws, gcp, azure, openstack, proxmox, ...)
 * @param clusterName       사용자가 부여한 클러스터 이름
 * @param apiServerUrl      kubeadm control-plane 의 https 엔드포인트 (port 6443)
 * @param masterPublicIp    master 의 public IP. Proxmox 등 사설망 only 환경에서는 빈 문자열 허용 → NotBlank 제외
 ...
 */
public record ProvisioningOutput(
