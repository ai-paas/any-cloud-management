...
public class AgentRuntimeEndpoint extends AgentRuntimeGrpc.AgentRuntimeImplBase {
    ...
    private AuthOutcome authenticateRequest() {
        ...
        recordAuthMetric(outcome.path(), outcome.identity().isPresent() ? "success" : "reject");
        return outcome;
    }
    ...
    private AuthOutcome authenticateRequestInternal() {
        ...
        if (token == null) {
            return new AuthOutcome(Optional.empty(), "bearer", "Authorization header missing");
        }
        Optional<AgentIdentity> id = authenticator.authenticate(token);
        return new AuthOutcome(id, "bearer", id.isEmpty() ? "identity_token invalid / revoked / expired" : null);
    }
    ...
    public StreamObserver<AgentMessage> stream(StreamObserver<ControlMessage> downstream) {
        ...
        if (outcome.identity().isEmpty()) {
            log.warn("Runtime stream auth rejected path={} reason={}", outcome.path(), outcome.failReason());
            downstream.onError(
                    Status.UNAUTHENTICATED.withDescription(outcome.failReason()).asRuntimeException());
            return new NoopAgentObserver();
        }
        AgentIdentity agent = outcome.identity().get();
        AgentSession session = sessionRegistry.register(agent.clusterName(), agent.agentInstanceId(), downstream);
        ...
        if (transitionedToActive) {
            try {
                ...
                log.warn("Failed to mark agent ACTIVE cluster={}: {}", agent.clusterName(), e.toString());
            }
        }
        AgentIdentity active = agent.withStatus(AgentStatus.ACTIVE, null);
        notifyListeners(l -> l.onStreamConnected(active));

        log.info("Runtime stream opened cluster={} instance={}", session.clusterId(), session.agentInstanceId());
    ...
    public StreamObserver<LogPacket> streamPodLogs(StreamObserver<LogPacket> toAgent) {
        AuthOutcome outcome = authenticateRequest();
        if (outcome.identity().isEmpty()) {
            log.warn("streamPodLogs auth rejected path={} reason={}", outcome.path(), outcome.failReason());
            toAgent.onError(
                    Status.UNAUTHENTICATED.withDescription(outcome.failReason()).asRuntimeException());
    ...
    private void updateAgentHeartbeat(String clusterName, Heartbeat heartbeat) {
        try {
            ...
            if (heartbeat != null
                    && heartbeat.hasHealth()
                    && heartbeat.getHealth().hasLastK8SApiOk()) {
    ...
    private class AgentObserver implements StreamObserver<AgentMessage> {
        ...
        public void onNext(AgentMessage msg) {
            try (MDC.MDCCloseable c = MDC.putCloseable("cluster", session.clusterId())) {
                ...
                log.error("Failed to dispatch AgentMessage cluster={}: {}", session.clusterId(), e.toString(), e);
            }
        }
        ...
        private void dispatch(AgentMessage msg) {
            ...
            switch (msg.getPayloadCase()) {
                ...
                case EVENT -> {
                    log.debug(
                            "AgentEvent cluster={} type={}",
                            session.clusterId(),
                            msg.getEvent().getEventType());
                    sessionRegistry.onUnsolicited(session, msg);
                    notifyListeners(l -> l.onAgentEvent(session.clusterId(), msg.getEvent()));
                }
                case PAYLOAD_NOT_SET -> log.debug("AgentMessage with no payload cluster={}", session.clusterId());
            }
        }
    }
    ...
    private static class NoopExecObserver implements StreamObserver<ExecPacket> {
        @Override
        public void onNext(ExecPacket value) {}
        ...
        public void onNext(ExecPacket pkt) {
            if (bridge == null) {
                if (pkt.getPayloadCase() != ExecPacket.PayloadCase.REQUEST) {
                    log.warn(
                            "PodExec: first packet not Request (cluster={}, case={})",
                            clusterName,
                            pkt.getPayloadCase());
                ...
                if (bridge == null) {
                    log.warn("PodExec: session {} unknown or already bound (cluster={})", sessionId, clusterName);
    ...
    private static class NoopLogStreamObserver implements StreamObserver<LogPacket> {
        @Override
        public void onNext(LogPacket value) {}
        ...
        public void onNext(LogPacket pkt) {
            if (bridge == null) {
                if (pkt.getPayloadCase() != LogPacket.PayloadCase.REQUEST) {
                    log.warn(
                            "StreamPodLogs: first packet not Request (cluster={}, case={})",
                            clusterName,
                            pkt.getPayloadCase());
                    toAgent.onError(Status.INVALID_ARGUMENT
                ...
                if (bridge == null) {
                    log.warn("StreamPodLogs: session {} unknown or already bound (cluster={})", sessionId, clusterName);
                    toAgent.onError(Status.NOT_FOUND
