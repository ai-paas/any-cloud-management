...
public class AgentSessionRegistry {
    ...
    /**
     * session → 마지막 응답 시각. timeout 이 "끊긴 연결" 인지 "느린 명령" 인지 가르는 유일한 신호다.
     *
     * <p>죽은 pod 의 stream 은 TCP keepalive 가 걷어갈 때까지 남아 leader 자리를 차지한다.
     */
    private final Map<AgentSession, Long> lastResponseAt = new ConcurrentHashMap<>();
    ...
    private final Cache<String, PendingCommand> pendingByRequest = Caffeine.newBuilder()
            ...
            .removalListener((String key, PendingCommand cmd, RemovalCause cause) -> {
                if (cmd != null
                    ...
                    cmd.future()
                            .completeExceptionally(new SessionClosedException(
    ...
    public AgentSession register(
            String clusterName, String agentInstanceId, StreamObserver<ControlMessage> downstream) {
        AgentSession session = new AgentSession(clusterName, agentInstanceId, downstream, System.currentTimeMillis());

        List<AgentSession> list = sessionsByCluster.computeIfAbsent(clusterName, k -> new CopyOnWriteArrayList<>());
        ...
        list.removeIf(s -> {
            if (agentInstanceId.equals(s.agentInstanceId())) {
                ...
                log.info("Stale session removed cluster={} instance={} (re-register)", clusterName, agentInstanceId);
                return true;
            }
            return false;
        });
        ...
        list.add(session);
        log.info(
                "Agent session registered cluster={} instance={} replicas={} (leader={})",
                clusterName,
                agentInstanceId,
                list.size(),
                list.get(0).agentInstanceId());
        return session;
    }
    ...
    public int evictByCluster(String clusterName, String reason) {
        ...
        String msg =
                "agent forcefully disconnected by admin" + (reason == null || reason.isBlank() ? "" : ": " + reason);
        for (AgentSession session : sessions) {
            try {
                session.downstream()
                        .onError(io.grpc.Status.PERMISSION_DENIED
                                .withDescription(msg)
                                .asRuntimeException());
            } catch (Exception e) {
                log.warn(
                        "Failed to send onError to session cluster={} instance={}: {}",
                        clusterName,
                        session.agentInstanceId(),
                        e.getMessage());
            }
     ...
     */
    public CompletableFuture<CommandResponse> sendCommand(
            String clusterName, ControlMessage.Builder controlBuilder, int timeoutSeconds) {
        ...
        if (leaderOpt.isEmpty()) {
            ...
            failed.completeExceptionally(
                    new NoActiveSessionException("No active agent session for cluster: " + clusterName));
            return failed;
        }
        ...
        pendingByRequest.put(requestId, new PendingCommand(clusterName, future, System.currentTimeMillis(), session));

        long submittedAt = System.currentTimeMillis();
        future.orTimeout(timeoutSeconds, TimeUnit.SECONDS).whenComplete((r, ex) -> {
            pendingByRequest.invalidate(requestId);
            if (ex instanceof java.util.concurrent.TimeoutException) {
                dropIfSilentSince(session, submittedAt);
            }
        });
    ...
    /**
     * 명령이 시한을 넘겼고 그 사이 이 stream 이 아무 응답도 주지 않았으면 끊어진 것으로 본다.
     *
     * <p>같은 stream 이 다른 명령에는 답하고 있다면 그저 느린 명령이다 — 무거운 작업마다 클러스터가
     * 끊기면 안 되므로 그때는 남겨둔다.
     */
    private void dropIfSilentSince(AgentSession session, long submittedAt) {
        Long answered = lastResponseAt.get(session);
        if (answered != null && answered >= submittedAt) {
            return;
        }
        log.warn(
                "응답 없는 agent stream 을 내림 cluster={} instance={} — 다음 replica 로 넘긴다",
                session.clusterId(),
                session.agentInstanceId());
        // 레지스트리에서만 빼면 agent 는 아직 붙어 있다고 믿는다. 끊은 줄 몰라 다시 연결하지도
        // 않으므로, 그 클러스터는 pod 가 재시작될 때까지 닿지 않는다.
        try {
            session.downstream()
                    .onError(io.grpc.Status.UNAVAILABLE
                            .withDescription("agent stream unresponsive — reconnect")
                            .asRuntimeException());
        } catch (Exception e) {
            log.debug("stream 종료 통보 실패 instance={}: {}", session.agentInstanceId(), e.toString());
        }
        unregister(session);
    }
    ...
    public void completeResponse(String requestId, CommandResponse response) {
        ...
        if (pending == null) {
            log.warn("Received response for unknown request_id={} (already timed out or unknown agent)", requestId);
            return;
        }
        pendingByRequest.invalidate(requestId);
        if (pending.leaderAtSubmit() != null) {
            lastResponseAt.put(pending.leaderAtSubmit(), System.currentTimeMillis());
        }
    ...
    public boolean openExecSession(String clusterName, String sessionId, ExecRequest request) {
        ...
        ControlMessage control = ControlMessage.newBuilder()
                ...
                .setOpenExecSession(
                        OpenExecSession.newBuilder().setSessionId(sessionId).setRequest(request))
                .build();
        try {
     ...
     * @return true 면 push 성공, false 면 active session 없음.
     */
    public boolean openLogStream(
            String clusterName, String sessionId, io.aipaas.cluster.agent.v1.LogStreamRequest request) {
        Optional<AgentSession> leaderOpt = find(clusterName);
        if (leaderOpt.isEmpty()) {
