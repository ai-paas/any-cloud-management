...
public class HelmReleaseService {
     ...
     */
    public InstalledRelease install(
            String clusterName,
            String releaseName,
            String chart,
            String repoUrl,
     ...
     */
    public InstalledRelease upgrade(
            String clusterName,
            String releaseName,
            String chart,
            String repoUrl,
    ...
    public boolean uninstall(
        ...
        CompletableFuture<CommandResponse> future =
                commandRouter.uninstallAddon(clusterName, releaseName, namespace, keepHistory, wait);
     ...
     */
    public java.util.List<HelmReleaseResource> listReleaseResources(
            String clusterName, String namespace, String release) {
        CompletableFuture<CommandResponse> future =
                commandRouter.listHelmReleaseResources(clusterName, namespace, release);
        ...
        for (Value v : itemsField.getListValue().getValuesList()) {
            ...
            out.add(new HelmReleaseResource(
                    getString(s, "kind"), getString(s, "apiVersion"), getString(s, "namespace"), getString(s, "name")));
        }
        return out;
    }
    ...
    public ReleaseStatus getStatus(String clusterName, String namespace, String release) {
        CompletableFuture<CommandResponse> future = commandRouter.getHelmReleaseStatus(clusterName, namespace, release);
     ...
     */
    public java.util.List<HistoryRevision> getHistory(String clusterName, String namespace, String release, int max) {
        CompletableFuture<CommandResponse> future =
                commandRouter.getHelmReleaseHistory(clusterName, namespace, release, max);
     ...
     */
    public ReleaseStatus rollback(String clusterName, String namespace, String release, int revision, boolean wait) {
        CompletableFuture<CommandResponse> future =
                commandRouter.rollbackHelmRelease(clusterName, namespace, release, revision, wait);
    ...
    public record HistoryRevision(
            int revision, String updated, String status, String chart, String appVersion, String description) {}
    ...
    public record InstalledRelease(
            String release, String namespace, String chart, String version, int revision, String status) {
    ...
    private static CommandResponse await(
            CompletableFuture<CommandResponse> future, String opName, long timeoutSeconds) {
        try {
    ...
    private static void requireOk(CommandResponse resp, String opName) {
        if (resp.getStatus() != Status.OK) {
            throw new HelmRoutingException("Agent " + opName + " returned " + resp.getStatus() + " ("
