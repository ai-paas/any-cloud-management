...
public class ClusterAgentBackupAutoConfiguration {
    ...
    public EtcdBackupService etcdBackupService(AgentSessionRegistry sessionRegistry, BackupProperties props) {
    ...
    @ConditionalOnMissingBean
    @ConditionalOnProperty(prefix = "cluster-backup.node", name = "enabled", matchIfMissing = true)
    public PkiBackupService pkiBackupService(AgentSessionRegistry sessionRegistry, BackupProperties props) {
    ...
    public VeleroInstaller veleroInstaller(
        ...
        return new VeleroInstallerImpl(
                helmReleaseService, backupPolicyInstaller, props.velero().autoInstallPolicies());
    }
