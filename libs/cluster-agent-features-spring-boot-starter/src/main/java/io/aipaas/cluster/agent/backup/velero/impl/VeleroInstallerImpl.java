...
public class VeleroInstallerImpl implements VeleroInstaller {
    ...
    public VeleroInstallerImpl(
            HelmReleaseService helmReleaseService, BackupPolicyInstaller policyInstaller, boolean autoInstallPolicies) {
    ...
    public VeleroInstallResult install(String clusterName, VeleroInstallSpec spec) {
        ...
        log.info(
                "Velero install cluster={} ns={} chart={} version={} provider={}",
                clusterName,
                ns,
                CHART_NAME,
                chartVersion,
        ...
        try {
            ...
            installResult = new VeleroInstallResult(
                    clusterName, result.release(), result.namespace(), result.version(), result.status());
        } catch (RuntimeException e) {
            throw new BackupException("VELERO_INSTALL_FAILED", "helm install velero failed: " + e.getMessage(), e);
        }
        ...
        if (autoInstallPolicies && policyInstaller != null) {
            try {
                ...
                long ok = results.stream()
                        .filter(r -> "applied".equals(r.status()))
                        .count();
                log.info("Velero policy auto-install cluster={} applied={}/{}", clusterName, ok, results.size());
                if (ok < results.size()) {
                    for (BackupPolicyApplyResult r : results) {
                        if (!"applied".equals(r.status())) {
                            log.warn("Velero policy {} status={} cluster={}", r.policyId(), r.status(), clusterName);
                        }
                    }
                }
            } catch (Exception e) {
                log.warn("Velero policy auto-install failed cluster={} (non-fatal): {}", clusterName, e.toString());
            }
        }
    ...
    private static Map<String, Object> bslConfig(VeleroInstallSpec spec) {
        ...
        switch (p) {
            ...
            case "gcp" -> {
                /* GCS 는 별도 config 없음 — credentials 만 */
            }
            case "azure" -> {
