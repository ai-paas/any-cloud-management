        ...
        try {
            ...
            if (resp.getStatus() != Status.OK) {
                throw new BackupException(
                        resp.getErrorCode().isEmpty() ? "BACKUP_FAILED" : resp.getErrorCode(), resp.getErrorMessage());
            }
            ...
            long size = (long) fields.getOrDefault(
                            "size_bytes",
                            Value.newBuilder().setNumberValue(payload.length).build())
                    .getNumberValue();
            ...
            throw new BackupException(
                    "TIMEOUT", "timeout waiting for backup response (cluster=" + clusterName + ")", e);
