...
public record VeleroScheduleRequest(
        String name, String namespace, String cron, VeleroBackupRequest template, boolean paused) {

    public static VeleroScheduleRequest dailyFull(String name, String cron) {
        return new VeleroScheduleRequest(
                name, "velero", cron, VeleroBackupRequest.fullCluster(name + "-template"), false);
    }
}
